          ...
          user.products.map((product, index) => {
            ...
            var tax_rate =
              product.tax_rate && product.tax_rate != 0
                ? parseFloat(1 + product.tax_rate / 100)
                : 0;

            var taxable_amount = tax_rate != 0 ? value / tax_rate : 0;

            var cgst_rate =
              product.cgst_rate && product.cgst_rate != 0
                ? product.cgst_rate / 100
                : 0;
            var sgst_rate =
              product.sgst_rate && product.sgst_rate != 0
                ? product.sgst_rate / 100
                : 0;
            var igst_rate =
              product.igst_rate && product.igst_rate != 0
                ? product.igst_rate / 100
                : 0;
            ...
            csvData1["Invoice No."] =
              user.invoice_no && "ATW-" + user.invoice_no;
            csvData1["Order Date."] = moment(user.createdAt).format("lll");
            csvData1["Invoice Date."] = moment(user.createdAt).format("lll");
            csvData1["Delivery Date."] =
              user.delivery_date &&
              user.delivery_date.time &&
              moment(user.delivery_date.time).format("lll");
            csvData1["Delivery Type."] = "Door Drop";
            csvData1["Delivery Agent"] =
              user.driver_id[0] && user.driver_id[0].username;
            csvData1["Delivery Agent Phone"] =
              user.driver_id[0] && user.driver_id[0].phone;
            ...
            csvData1["Product Code"] =
              product.inventory_id.inventory_product_code;
            csvData1["Product Name-Inventory Name"] =
              product.product_id.name +
              "-" +
              product.inventory_id.inventory_name;
            ...
            csvData1["Product Sub Category"] =
              product.product_id.sub_category_id.name;
