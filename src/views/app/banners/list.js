...
class BannerList extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      addNewItemRoute: "/app/add-banner",
    ...
    swalWithBootstrapButtonsStatus
      ...
      .then(async (result) => {
        if (result.value) {
          ...
          let path = ApiRoutes.UPDATE_BANNER_STATUS + "/" + itemId;
  ...
  onDeleteItem = async (itemId, index) => {
    swalWithBootstrapButtonsDelete
      ...
      .then(async (result) => {
        if (result.value) {
          let path = ApiRoutes.DELETE_BANNER + "/" + itemId;
                      ...
                      {this.state.items.map((item, index) => {
                        return (
                          <tr key={index}>
                            ...
                            <td>
                              <img alt={item.title} src={item.banner_image_thumb_url} className="img-thumbnail border-0 list-thumbnail align-self-center xsmall" />
                            </td>
                            <td>{item.title}</td>
                            <td>{item ? item.product.inventory_name : ""}</td>
