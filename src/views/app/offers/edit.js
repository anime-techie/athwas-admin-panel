...
class EditOffer extends Component {
    ...
    if (res) {
      if (res.status == 200) {
        this.setState({
          ...
          business_category: res.data.business_category_id[0]
            ? res.data.business_category_id[0]._id
            : res.data.business_category_id,
          product_category: res.data.category[0]
            ? res.data.category[0]._id
            : res.data.categor,
          product_subcategory: res.data.subcategory[0]
            ? res.data.subcategory[0]._id
            : res.data.subcategory,
          product_inv_id: res.data.product_id
            ? res.data.product_id
            : res.data.product_id,
  ...
  getProducts = async (
    ...
    options = [];
    this.setState({ options: [{ value: "", label: "Select" }] });
    reactSelect = [];
        ...
        if (res.status == 200) {
          ...
          res.data.map((item, index) => {
            options.push({
              // key: index,
              value: item.id,
              label: item.name + "-" + item.inventory_name,
            });
            this.setState({ options: options });
            console.log("dsjhkjsf", this.state.option);
            // setFieldValue && setFieldValue("options", options);
  ...
  handleSubmit = async (inputValues, formOptions) => {
    var error = "";
    if (
      inputValues.offer_type == 2 ||
      inputValues.offer_type == 3 ||
      inputValues.offer_type == 4
    ) {
    ...
    if (
      inputValues.offer_amount != "" &&
      inputValues.offer_amount != null &&
      parseFloat(inputValues.offer_amount) < parseFloat(inputValues.offer_price)
    ) {
      formOptions.setFieldError(
        "offer_price",
        "Offer Price cannot be greater than Offer Amount"
      );
      return false;
    }
    if (
      inputValues.offer_amount_type != null &&
      inputValues.offer_amount_type != "" &&
      inputValues.offer_amount_type == "2" &&
      parseFloat(inputValues.offer_price) > 100
    ) {
      formOptions.setFieldError(
        "offer_price",
        "Offer Price percentage should not be greater than 100 %"
      );
      return false;
    }
    ...
    formData.append(
      ...
      typeof inputValues.business_category !== "undefined"
        ? inputValues.business_category
        : null
    );
    formData.append(
      "category_id",
      typeof inputValues.product_category !== "undefined"
        ? inputValues.product_category
        : ""
    );
    formData.append(
      "sub_category_id",
      typeof inputValues.product_subcategory !== "undefined"
        ? inputValues.product_subcategory
        : null
    );
    formData.append(
      "product_id",
      typeof inputValues.product_inv_id !== "undefined"
    ...
    formData.append(
      "offer_amount_type",
      inputValues.offer_amount_type != null ? inputValues.offer_amount_type : ""
    );
    formData.append(
      "offer_price",
      inputValues.offer_price != "" ? inputValues.offer_price : ""
    );
    formData.append(
      "offer_amount",
      inputValues.offer_amount != null ? inputValues.offer_amount : ""
    );

    formData.append(
      "offer_product",
      inputValues.offer_product ? inputValues.offer_product : ""
    );
    formData.append(
      "offer_quantity",
      inputValues.offer_quantity ? inputValues.offer_quantity : ""
    );
                              ...
                              <FormGroup className="form-group has-float-label">
                                ...
                                <select
                                  ...
                                  onChange={(event) => {
                                    ...
                                    this.getPerentCategories(
                                      event.target.value
                                    );
                                  }}
                                >
                                  {this.state.businessCatList.map(
                                    (item, index) => {
                                      return (
                            ...
                            <Colxx xxs="12" sm="6">
                              <FormGroup className="form-group has-float-label">
                                ...
                                <FormikReactSelect
                                  ...
                                  isMulti={
                                    this.state.offer_type == 2 ? true : false
                                  }
