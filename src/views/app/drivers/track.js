...
const AnyReactComponent = ({ text }) => (
  <div>
    <i
      class="glyph-icon iconsminds-map-marker-2"
      style={{ "font-size": "32px", color: "red", "line-height": "42px" }}
    ></i>
    {text}
  </div>
);
...
class renderMap extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      user_name:
        (this.props.location.state.data &&
          this.props.location.state.data.username) ||
        "",
  ...
  renderMarkers(map, maps) {
    ...
    rootRef.on("value", (snap) => {
      if (snap.child("lat").val()) {
        ...
        bounds.extend(
          new maps.LatLng(snap.child("lat").val(), snap.child("lng").val())
        );
